...
// Variable global para gestionar la lista de procesos y la sincronización
...
sem_t sem_rpta_estructura_inicializada; // Semáforo para sincronizar la respuesta de memoria
...
// Función principal del PLP que será llamada cuando el kernel se inicie
void iniciar_plp() {
    char tecla;
...
// Función que implementa el algoritmo del Planificador de Largo Plazo
void planificadorLargoPlazo(t_log* logger) {
    pcb_t* pcb = NULL;
    
    pthread_mutex_lock(&mutex_lista_new);  // Bloqueamos la lista de procesos en NEW

    // Si la lista de NEW no está vacía, comenzamos a procesar
    if (!list_is_empty(lista_new)) {
        ...
        if (pcb != NULL) {
            ...
            add_string_to_buffer(buffer, pcb->path);
            ...
            log_info(logger, "Se aviso a Memoria para la creacion del proceso");
            ...
            if (flag_pedido_de_memoria) {
                ...
                log_info(logger, "Creación de Proceso: ## (%d:0) Se crea el proceso - Estado: NEW", pcb->pid);
                agregar_pcb_lista(pcb, lista_ready, mutex_lista_ready);
                cambiar_estado(pcb, READY_PROCCES);  // Cambiamos su estado a READY
                log_info(logger, "Creación de Proceso: ## (%d:0) Se cambia el estado a READY", pcb->pid);
            }
