...
void crear_conexiones(t_log* logger, t_config* config){
   ...
   datos_servidor_t* datos = malloc(sizeof(datos_servidor_t));
    ...
    datos->puerto = config_get_string_value(config, "PUERTO_ESCUCHA_IO");
    ...
    datos_dispatch->logger = logger;
    datos_dispatch->nombre_servidor = "Kernel Dispatch";
    datos_dispatch->puerto = config_get_string_value(config, "PUERTO_ESCUCHA_DISPATCH");
    ...
    datos_interrupt->logger = logger;
    datos_interrupt->nombre_servidor = "Kernel Interrupt";
    datos_interrupt->puerto = config_get_string_value(config, "PUERTO_ESCUCHA_INTERRUPT");
    ...
    datosConexion->ip = config_get_string_value(config, "IP_MEMORIA");
    datosConexion->puerto = config_get_string_value(config, "PUERTO_MEMORIA");
